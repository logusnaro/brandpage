...
## 2026-08-29 — Galaxy S26 모바일 레이아웃 교정

### 원인

- Intro 이미지, 본문, 보조문구, 진행바가 모바일에서도 절대좌표를 공유해 서로 겹침.
- 이미지가 화면 대부분을 차지해 하단 문구 영역과 충돌.

### 처리 내용

- 모바일 Intro를 상단 이미지, 하단 문구, 최하단 섹션 네비게이션 영역으로 분리.
- 보조문구와 Intro 진행바를 문구 패널의 정상 흐름으로 복구.
- 모바일 이미지 높이와 문구 패널 여백을 S26 화면에 맞게 조정.

### 검증 결과

- 실제 Galaxy S26 연결 화면에서 기존 겹침 현상 확인.
- 360×695 및 412×780 뷰포트에서 Intro 1~5 장면 좌표 충돌 없음 확인.
- Product 가로 카드와 Contact 폼 모바일 배치 확인.
- `npm run build` 및 TypeScript 검사 통과.
